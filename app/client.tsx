...
export default function Client({
  ...
  useEffect(() => {
    if (days) {
      ...
      console.log(
        "🚀 ~ file: client.tsx:29 ~ SevenDayForecast:",
        JSON.stringify(SevenDayForecast)
      );
    ...
    <section>
      <div>
        {data && (
          <>
            <p>
              Current weather for {searchParams.city}, {searchParams.region}
            </p>
      ...
      <div className="flex min-h-screen flex-row items-center justify-between p-24">
        {forecast?.map(
          (
            element: { days: string; highTemp: string; lowTemp: string },
            i: number
          ) => {
            return (
              <div key={i}>
